[![Typing SVG](https://readme-typing-svg.demolab.com?font=Mystery+Quest&size=40&pause=1000&color=7B00F7&background=FFFFFF&center=true&vCenter=true&width=435&lines=Hi+there+and+welcome)](https://git.io/typing-svg)

⚡ The Digital Pulse

    Identity: Bader

    Engine: Powered by a relentless passion for Technology.

    Mission: Architecting the future at the intersection of code, creativity, and intelligence
    
✨ My Creative Tech Journey ✨

    🎮 **The Player's Heart** (url)

        Immersed in the world of Video Games.

    🏗️ The Dream

        Building worlds through Game Development.

        Leading the vision as a Creative Director.

    🛡️ The Guardian

        Protecting the digital realm with Cybersecurity.

    🧠 The Future-Seeker

        Exploring the frontiers of AI Learning.

    🧬 The Architect of Intelligence

        ↳ The Ultimate Merge: Integrating AI + Cybersecurity to build autonomous, self-defending systems.
        
        💻 Tech Interest in Progress

    The Core: Sculpting logic with C++.

    The Intelligence: Scripting the future in Python.

    The Interface: Crafting the web via JavaScript.

    [![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=noneeeed&theme=radical)](https://github.com/anuraghazra/github-readme-stats)
    [![GitHub Streak](https://streak-stats.demolab.com?user=noneeeed&theme=radical&hide_border=true)](https://git.io/streak-stats)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Mystery+Quest&size=40&pause=1000&color=7B00F7&background=FFFFFF&center=true&vCenter=true&width=435&lines=I+Could+Be+Your+F1+Button)](https://git.io/typing-svg)


